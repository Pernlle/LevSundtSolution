...
namespace LevSundt.Bmi.Infrastructor.Repositorys
{
    public class BmiRepository : IBmiRepository
    {
        ...
        IEnumerable<BmiQueryResultDto> IBmiRepository.GetAll(string userId)
        {
            foreach (var entity in _db.BmiEntities.AsNoTracking().Where(a=> a.UserId==userId).ToList())
            {
                yield return new BmiQueryResultDto 
                    { Bmi = entity.Bmi, Weight = entity.Weight, Height = entity.Height, Id = entity.Id, Date = entity.Date, WeightCategori = entity.WeightCategori, RowVersion = entity.RowVersion };
            }           
        }
        ...
        BmiQueryResultDto IBmiRepository.Get(int id, string userId)
        {
            ...
            { Bmi = dbEntity.Bmi, Weight = dbEntity.Weight, Height = dbEntity.Height, Id = dbEntity.Id, Date = dbEntity.Date, WeightCategori = dbEntity.WeightCategori, RowVersion = dbEntity.RowVersion };
        }
    }
}
